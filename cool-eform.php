...
define( 'WPCEF_PLUGIN_VERSION', '0.3.0' );
    ...
    class WP_Cool_EForm {
        ...
        public function render_form() {
            if ( isset( $_GET['form_sent'] ) ) {
                include( WPCEF_PLUGIN_DIR . '/templates/sent.php' );
            } else {
        ...
        public function process_form() {
            ...
            if ( !empty( $submitted ) ) {
                ...
                $from = $this->get_from( $_POST, $general_settings['from_name'], $general_settings['from_address'] );
                ...
                if ( !$sent ) {
                    $message_1 = __( 'An error has occurred while sending the form.', 'cef' );
                    $message_2 = __( 'Please try it later or send the message directly to %s.', 'cef' );
                    $this->errors['form'] = $message_1 . ' ' . sprintf( $message_2, $to_address );
                    return;
                }

                $redirect_url = add_query_arg( 'form_sent', true, cef_get_param_value( 'cef-referrer' ) );
        ...
        private function get_from( $values, $default_from_name, $default_from_address ) {
            $from_address = isset( $values['cef-email'] ) ? trim( $values['cef-email'] ) : '';
            if ( empty( $from_address ) ) {
                $from_address = $default_from_address;
            }

            $from_name = isset( $values['cef-name'] ) ? trim( $values['cef-name'] ) : '';
            if ( empty( $from_name ) ) {
                // Since 'From' name setting is optional, it can be also empty
                if ( empty ( $default_from_name ) ) {
                    // In such a case return just email address
                    return $from_address;
                }
